 ...
 */
export const createInvoice = (order, res) => {
    ...
    for (const item of order.items) {
        ...
        generateTableRow(doc, position, item.product.images[0], item.product.name, item.qty.toString(), `$${price.toFixed(2)}`, `$${itemTotal.toFixed(2)}`);
...
function generateTableRow(doc, y, imagePath, item, quantity, rate, amount) {
    doc.fontSize(10);
    if (imagePath) {
